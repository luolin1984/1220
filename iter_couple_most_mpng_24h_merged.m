...
% ===================== [SAFE DEFAULTS] =====================
% evaluate_cap 在训练循环中高频调用本函数。这里先填充 out 的安全默认值，
% 避免上游 try/catch 失败时缺字段导致 obs 直接饱和。
out = struct();
out.ok      = false;
out.most_ok = false;
out.mpng_ok = false;
...
out.err     = '';
out.kpis    = struct();
out.kpis.avg_cost            = 1.0;
out.kpis.curtail_ratio       = 1.0;
out.kpis.gas_risk            = 1.0;
out.kpis.voltage_dev_avg_pu  = 0.20;
out.kpis.voltage_dev_max_pu  = 0.20;
out.kpis.voltage_dev_src     = "init";
...
try
    ...
    % vdev 评估口径（支持：'acpf'|'acopf'|'mpng'）
    vdev_eval = 'acpf';
    if isfield(opts,'vdev_eval') && ~isempty(opts.vdev_eval), vdev_eval = opts.vdev_eval; end
    if isfield(opts,'vdev') && isstruct(opts.vdev) && isfield(opts.vdev,'eval') && ~isempty(opts.vdev.eval)
        vdev_eval = opts.vdev.eval;
    end

    % 优先：若 MPNG 已成功且返回 eg.mpc.bus，则直接从耦合后的电侧 Vm 计算（最稳、最快）
    use_mpng_vm = false;
    if any(strcmpi(string(vdev_eval), ["mpng","acopf","acpf"]))
        if exist('mpng_ok','var') && mpng_ok && exist('eg','var') && isstruct(eg) && isfield(eg,'mpc') ...
                && isstruct(eg.mpc) && isfield(eg.mpc,'bus') && isnumeric(eg.mpc.bus) && ~isempty(eg.mpc.bus)
            use_mpng_vm = true;
        end
    end
    ...
    if use_mpng_vm
        ...
        [obj_vdev, vdev_kpi] = calc_voltage_deviation_from_mpng_bus(eg.mpc.bus, ID, T, vref, vdev_fail_pen, NB_base);
    else
        % 回退：用每小时 ACPF 后处理（原逻辑）
        [obj_vdev, vdev_kpi] = calc_voltage_deviation_acpf(mpc_used, Pg2, ID, T, vref, vdev_fail_pen);
        if ~isfield(vdev_kpi,'src'), vdev_kpi.src = "acpf"; end
    end
    ...
    % --- [DRL] 标准化 KPI 名称（供 evaluate_cap 取值） ---
    % 1) 平均成本（$/MWh）
    if isfield(out.kpis,'avg_cost_per_MWh_USD') && isfinite(out.kpis.avg_cost_per_MWh_USD)
        out.kpis.avg_cost = out.kpis.avg_cost_per_MWh_USD;
    end

    % 2) 弃风弃光比例（curtail/avail）
    if isfield(out.kpis,'curtail_MWh') && isfield(out.kpis,'ren_avail_MWh') && isfinite(out.kpis.ren_avail_MWh)
        out.kpis.curtail_ratio = max(0, out.kpis.curtail_MWh / max(1e-9, out.kpis.ren_avail_MWh));
    end

    % 3) 气网风险（优先用压力越限比例；若缺失，用最大越限幅度兜底）
    out.kpis.gas_risk = 0.0;
    try
        g = out.kpis.gas;
        if isstruct(g) && isfield(g,'press_violation_ratio') && isfinite(g.press_violation_ratio)
            out.kpis.gas_risk = min(max(g.press_violation_ratio, 0), 1);
        elseif isstruct(g) && isfield(g,'press_violation_max_pu') && isfinite(g.press_violation_max_pu)
            % max_pu 通常是“超上/超下占额定”的比例，做一个软映射到 [0,1]
            out.kpis.gas_risk = min(max(g.press_violation_max_pu, 0), 1);
        else
            out.kpis.gas_risk = 0.0;
        end
    catch
    ...
    % 4) 电压偏差标量（第4目标）
    if isfield(out.kpis,'voltage_dev') && isstruct(out.kpis.voltage_dev) && isfield(out.kpis.voltage_dev,'avg') && isfinite(out.kpis.voltage_dev.avg)
        out.kpis.voltage_dev_avg_pu = out.kpis.voltage_dev.avg;
    end
    if isfield(out.kpis,'voltage_dev') && isstruct(out.kpis.voltage_dev) && isfield(out.kpis.voltage_dev,'max') && isfinite(out.kpis.voltage_dev.max)
        out.kpis.voltage_dev_max_pu = out.kpis.voltage_dev.max;
    end
    if isfield(out.kpis,'voltage_dev') && isstruct(out.kpis.voltage_dev) && isfield(out.kpis.voltage_dev,'src')
        out.kpis.voltage_dev_src = string(out.kpis.voltage_dev.src);
    end
    ...
    % 不向外抛异常（否则 evaluate_cap 会直接进入 exception 分支）
    try
        out.err = getReport(ME, "basic", "hyperlinks", "off");
    catch
        out.err = ME.message;
    end
    warning('[ITER] exception: %s', ME.message);
end
end % ===== 主函数结束 =====

%% ================== 工具函数 ==================

function [obj_vdev, kpi] = calc_voltage_deviation_from_mpng_bus(bus_all, ID, T, vref, fail_pen, NB_base)
% 从 MPNG/耦合结果的 eg.mpc.bus 中直接提取 Vm，计算电压偏差
% 形状自适应：
%   - 若 size(bus_all,1) 可被 T 整除：认为是 (NB_per × T) 堆叠
%   - 否则认为是单时段快照，用同一 vdev 填满 T（用于 RL 稳定反馈）
%
% 返回：
%   obj_vdev = avg(vdev_per_hour)
%   kpi.per_hour(T×1), kpi.avg, kpi.max, kpi.failed_hours, kpi.src

if nargin < 6 || isempty(NB_base), NB_base = 0; end
if nargin < 5 || isempty(fail_pen), fail_pen = 0.20; end
if nargin < 4 || isempty(vref),     vref     = 1.0; end
if nargin < 3 || isempty(T),        T        = 24; end

define_constants;  %#ok<*DEFNU>
VM_COL  = VM;
BT_COL  = BUS_TYPE;

% 若 ID 带 VM/BUS_TYPE，则优先用 ID（兼容你代码里的索引封装）
if isstruct(ID)
    if isfield(ID,'VM') && isnumeric(ID.VM), VM_COL = ID.VM; end
    if isfield(ID,'BUS_TYPE') && isnumeric(ID.BUS_TYPE), BT_COL = ID.BUS_TYPE; end
end
...
nb_total = size(bus_all,1);
if nb_total <= 0
    kpi.src = "mpng:empty_bus";
    kpi.avg = fail_pen; kpi.max = fail_pen;
    obj_vdev = kpi.avg;
    return;
end
...
if mod(nb_total, T) == 0
    nb_per = nb_total / T;
    for tt = 1:T
        rr = (tt-1)*nb_per + (1:nb_per);
        Vm = bus_all(rr, VM_COL);
        bt = bus_all(rr, BT_COL);

        mask = isfinite(Vm) & (bt ~= NONE);
        if ~any(mask)
            ...
            if ~isfinite(kpi.per_hour(tt)), kpi.per_hour(tt) = fail_pen; kpi.failed_hours = kpi.failed_hours + 1; end
        end
    end
    kpi.src = sprintf("mpng:stack_by_T:nb_per=%d:nb_total=%d", nb_per, nb_total);
else
    % 2) 单时段快照：用同一 vdev 填满（用于 RL 的第4目标仍可提供梯度）
    Vm = bus_all(:, VM_COL);
    bt = bus_all(:, BT_COL);
    mask = isfinite(Vm) & (bt ~= NONE);
    ...
    if ~any(mask)
        ...
        kpi.src = sprintf("mpng:single_snapshot:no_valid_vm:nb_total=%d", nb_total);
    else
        dv = Vm(mask) - vref;
        v  = sqrt(mean(dv.^2));
        if ~isfinite(v), v = fail_pen; end
        kpi.per_hour(:) = v;
        kpi.src = sprintf("mpng:single_snapshot:nb_total=%d", nb_total);
    end
end
...
% 额外提示（便于 LIVE 输出定位）
if NB_base > 0 && mod(nb_total, NB_base) ~= 0
    kpi.src = string(kpi.src) + ":nb_not_divisible_by_base";
end
end
...
if NB_g <= 0
    % 容错：无法识别时，默认 48 节点（case48）避免训练期直接异常
    NB_g = 48;
    warning('[MPNG-NORM] detect_gas_nodes failed; assume NB_g=48.');
end
